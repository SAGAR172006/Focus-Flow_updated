...
const PdfEditor = () => {
  ...
  const handleFileUpload = async (e) => {
    ...
    if (uploadedFile && uploadedFile.type === "application/pdf") {
      ...
      setPdfDoc(pdf);
      alert("PDF loaded successfully!"); // Replaced toast
    } else {
      alert("Please upload a valid PDF file"); // Replaced toast
    }
  };
  ...
  const handleCanvasClick = (e) => {
    ...
    if (editMode === "text" && textInput) {
      ...
      setTextInput("");
      alert("Text added!"); // Replaced toast
      ...
      setAnnotations([...annotations, newAnnotation]);
      alert("Highlight added!"); // Replaced toast
    }
  };
  ...
  const handleImageUpload = (e) => {
    ...
    if (imageFile) {
      ...
      reader.onload = (event) => {
        ...
        setAnnotations([...annotations, newAnnotation]);
        alert("Image added!"); // Replaced toast
      };
  ...
  const deleteAnnotation = (id) => {
    ...
    alert("Annotation deleted!"); // Replaced toast
  };

  const savePdf = async () => {
    if (!pdfDoc) {
      alert("No PDF loaded"); // Replaced toast
      return;
    }
    ...
    try {
      ...
      alert("PDF saved successfully!"); // Replaced toast
    } catch (error) {
      console.error("Error saving PDF:", error);
      alert("Failed to save PDF"); // Replaced toast
    }
  };
        ...
        annotations
          ...
          .forEach((ann) => {
            if (ann.type === "text" && ann.content) {
              ctx.font = `${(ann.fontSize || 16) * scale}px ${
                ann.fontFamily || "Helvetica"
              }`;
